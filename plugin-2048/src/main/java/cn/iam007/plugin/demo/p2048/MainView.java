...
public class MainView extends View {
    ...
    private void drawDrawable(
            Canvas canvas, Drawable draw, int startingX, int startingY,
    ...
    private void drawCellText(Canvas canvas, int value, int sX, int sY) {
        ...
        if (value >= 8) {
            paint.setColor(mResources.getColor(R.color.text_white));
        } else {
            paint.setColor(mResources.getColor(R.color.text_black));
        }
    ...
    private void drawScoreText(Canvas canvas) {
        ...
        paint.setColor(mResources.getColor(R.color.text_brown));
        canvas.drawText(mResources.getString(R.string.high_score),
                sXHighScore + textMiddleHighScore,
                titleStartYAll,
                paint);
        paint.setTextSize(bodyTextSize);
        paint.setColor(mResources.getColor(R.color.text_white));
        ...
        paint.setTextSize(titleTextSize);
        paint.setColor(mResources.getColor(R.color.text_brown));
        canvas.drawText(mResources.getString(R.string.score), sXScore
                + textMiddleScore, titleStartYAll, paint);
        paint.setTextSize(bodyTextSize);
        paint.setColor(mResources.getColor(R.color.text_white));
        canvas.drawText(String.valueOf(game.score),
                sXScore + textMiddleScore,
                bodyStartYAll,
                paint);
    }
    ...
    private void drawHeader(Canvas canvas) {
        ...
        canvas.drawText(mResources.getString(R.string.header),
                startingX,
                headerStartY,
                paint);
    }
    ...
    private void drawInstructions(Canvas canvas) {
        ...
        paint.setTextAlign(Paint.Align.LEFT);
        int textShiftY = centerText() * 2;
        canvas.drawText(mResources.getString(R.string.instructions),
    ...
    private void drawBackgroundGrid(Canvas canvas) {
        ...
        Drawable backgroundCell = mResources.getDrawable(R.drawable.cell_rectangle);
        ...
        for (int xx = 0; xx < game.numSquaresX; xx++) {
            for (int yy = 0; yy < game.numSquaresY; yy++) {
                ...
                if (currentTile != null) {
                    ...
                    ArrayList<AnimationCell> aArray = game.aGrid.getAnimationCell(xx,
                            yy);
                    ...
                    for (int i = aArray.size() - 1; i >= 0; i--) {
                        ...
                        if (aCell.getAnimationType() == MainGame.SPAWN_ANIMATION) { // Spawning animation
                            ...
                            float textScaleSize = (float) (1 + INITIAL_VELOCITY
                                    * percentDone
                                    + MERGING_ACCELERATION * percentDone
                                    * percentDone / 2);
                            paint.setTextSize(textSize * textScaleSize);
                            ...
                            int dY = (int) ((currentY - previousY)
                                    * (cellSize + gridWidth)
                                    * (percentDone - 1) * 1.0);
                            bitmapCell[tempIndex].setBounds(sX + dX,
                                    sY + dY,
                                    eX + dX,
                                    eY + dY);
    ...
    private void drawEndlessText(Canvas canvas) {
        ...
        paint.setTextSize(bodyTextSize);
        paint.setColor(mResources.getColor(R.color.text_black));
        canvas.drawText(mResources.getString(R.string.endless),
                startingX,
                sYIcons - centerText() * 2,
                paint);
    }

    private void createEndGameStates(
            Canvas canvas, boolean win, boolean showButton) {
        ...
        if (win) {
            ...
            canvas.drawText(mResources.getString(R.string.you_win),
                    middleX,
                    textBottom,
                    paint);
            paint.setTextSize(bodyTextSize);
            String text = showButton ? mResources.getString(R.string.go_on)
                    : mResources.getString(R.string.for_now);
            ...
            paint.setTextSize(gameOverTextSize);
            paint.setTextAlign(Paint.Align.CENTER);
            canvas.drawText(mResources.getString(R.string.game_over),
                    middleX,
                    middleY - centerText(),
                    paint);
        }
    ...
    private void createBitmapCells() {
        ...
        for (int xx = 1; xx < bitmapCell.length; xx++) {
            ...
            float tempTextSize = cellTextSize
                    * cellSize
                    * 0.9f
            ...
            drawDrawable(canvas,
                    mResources.getDrawable(cellRectangleIds[xx]),
                    0,
                    0,
                    cellSize,
                    cellSize);
            drawCellText(canvas, value, 0, 0);
            bitmapCell[xx] = new BitmapDrawable(mResources.getResources(),
                    bitmap);
        }
    }
    ...
    private void createOverlays() {
        Resources resources = mResources.getResources();
        //Initalize overlays
        ...
        bitmap = Bitmap.createBitmap(endingX - startingX,
                endingY - startingY,
                Bitmap.Config.ARGB_8888);
    ...
    private void getLayout(int width, int height) {
        ...
        titleWidthHighScore = (int) (paint.measureText(mResources.getString(R.string.high_score)));
        titleWidthScore = (int) (paint.measureText(mResources.getString(R.string.score)));
        paint.setTextSize(bodyTextSize);
    ...
    public MainView(Context context, PluginResourceLoader res) {
        ...
        try {
            ...
            Typeface font = Typeface.createFromAsset(mResources.getAssets(),
                    "ClearSans-Bold.ttf");
